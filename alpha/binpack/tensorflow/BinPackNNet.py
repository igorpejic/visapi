    ...
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            if TRAIN_ON_FIRST_STATE:
                ...
                # s: batch_size x board_x x board_y
                self.input_boards = tf.placeholder(tf.float32, shape=[None, self.channels, self.board_x, self.board_y])
            self.dropout = tf.placeholder(tf.float32)
            self.isTraining = tf.placeholder(tf.bool, name="is_training")

            # batch_size  x board_x x board_y x 1
            x_image = tf.reshape(self.input_boards, [-1, self.board_x, self.board_y, self.channels])
            # batch_size  x board_x x board_y x num_channels
            h_conv1 = Relu(BatchNormalization(self.conv2d(x_image, args.num_channels, 'same'), axis=3, training=self.isTraining))
            # batch_size  x board_x x board_y x num_channels
            h_conv2 = Relu(BatchNormalization(self.conv2d(h_conv1, args.num_channels, 'same'), axis=3, training=self.isTraining))
            # batch_size  x (board_x-2) x (board_y-2) x num_channels
            h_conv3 = Relu(BatchNormalization(self.conv2d(h_conv2, args.num_channels, 'valid'), axis=3, training=self.isTraining))

            # batch_size  x (board_x-4) x (board_y-4) x num_channels
            h_conv4 = Relu(BatchNormalization(self.conv2d(h_conv3, args.num_channels, 'valid'), axis=3, training=self.isTraining))
